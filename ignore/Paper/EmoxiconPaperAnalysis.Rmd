...
# library(knitr)
# purl("ignore/Paper/EmoxiconPaperAnalysis.Rmd", output = "EmoxiconPaperAnalysis-July-13-2020.R", documentation = 0) # Extract R code from Rmd document and discard all text chunks

modified_file <- "/Users/tara/Documents/emoxicon/ignore/Paper/EmoxiconPaper.Rmd"
# reference_file <- system.file("/Users/tara/Desktop/Consulting/SODH/merge data/sdoh_data_creation.Rmd", package = "reviewer")

result <- diff_rmd(current_file = modified_file)
library(reviewer)
library(papaja)
library(emoxicon)
library(eRm)
...
pparameters <- eRm::person.parameter(trolls_models$full_model, extrapolated = TRUE)
...
utests<-lapply(colnames(trolls_models$category_order), function(i){
  wilcox.test(trolls_models$category_order[catright,i],
         trolls_models$category_order[catleft,i])
})
names(utests) <- colnames(trolls_models$category_order)
# pvalues
cat("\n pvalues for Mann–Whitney U test \n")
sapply(1:length(colnames(trolls_models$category_order)), function(num){
  rbind(colnames(trolls_models$category_order)[num],
        round(utests[[num]]$p.value,3))
})
